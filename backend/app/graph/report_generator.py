...
class LLMReportGenerator:
    ...
    def generate(self, facts: ReportFacts) -> ExperimentReport:
        ...
        try:
            text, llm_usage = self._generate_text(facts, confidence, stars, base_reason, significant_results)
    ...
    def _generate_text(self, facts: ReportFacts, confidence: ConfidenceLevel, stars: int, base_reason: str, significant: list[StatResult]):
        """
        Returns `(parsed_output, llm_usage)` — `parsed_output` is the
        `_ReportLLMOutput` Pydantic instance (as before), `llm_usage`
        is an `LLMUsage | None` (see its docstring in
        app/schemas/execution.py) describing the token/cost accounting
        for this call, extracted by `invoke_structured_with_usage`
        below. `generate()` unpacks both and copies `llm_usage`
        straight onto `ExperimentReport.llm_usage`.
        """
        from pydantic import BaseModel, Field

        from app.llm.client import get_llm, invoke_structured_with_usage
